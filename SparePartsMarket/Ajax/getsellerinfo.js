document.getElementById('grab2').addEventListener('click',showValues);

         function showValues() {
             var req = new XMLHttpRequest();
             req.open('POST','/SparePartsMarket/PHP Handler/getsellerinfo.php');
             req.onload = function() {
                 if(req.status == 200){
                     ...
                     for(var i in details){
                         ...
                         '<li> shopname: '+details[i].shopname+'</li>'+
                         '<li> shoplocation: '+details[i].shoplocation+'</li>'+
